...
// ================== DATABASE ==================
// Use /tmp for Render writable database
...
db.serialize(() => {
  if (!dbExists) {
    ...
    db.run(`INSERT OR IGNORE INTO admin(username,password) VALUES ('admin','kameshthehacker')`);
    console.log("✅ Database initialized in /tmp with default admin (admin/admin123).");
  }
});
...
app.put("/api/challenges/:id", adminAuth, (req, res) => {
  ...
  const { title, category, description, flag, points, link } = req.body;
  db.run(
    `UPDATE challenges SET title=?, category=?, description=?, flag=?, points=?, link=? WHERE id=?`,
